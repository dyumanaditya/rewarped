###########################################################################
# Example Sim Grad Push
#
# Shows how to use Warp to optimize the initial velocity of a body
# such that it reaches a target position.
#
# This example uses the built-in wp.Tape() object to compute gradients of
# the distance to target (loss) w.r.t the initial velocity, followed by
# a simple gradient-descent optimization step.
#
###########################################################################

import numpy as np
import torch
import math

import warp as wp

from rewarped.environment import IntegratorType, run_env
from rewarped.warp_env import WarpEnv
from rewarped.utils.plotter import Plotter


class Slide(WarpEnv):
    sim_name = "Slide" + "WarpExamples"
    env_offset = (0.0, 0.0, 2.5)

    # integrator_type = IntegratorType.EULER
    # sim_substeps_euler = 8
    # euler_settings = dict(angular_damping=0.0)

    integrator_type = IntegratorType.FEATHERSTONE
    sim_substeps_featherstone = 8
    featherstone_settings = dict(angular_damping=0.0, update_mass_matrix_every=1)

    eval_fk = True
    eval_ik = False

    # frame_dt = 1.0 / 240.0
    frame_dt = 1.0 / 60.0
    # episode_duration = 1.6
    episode_duration = 0.6
    up_axis = "Y"
    ground_plane = True

    state_tensors_names = ("joint_q", "joint_qd")

    def __init__(self, num_envs=8, episode_length=-1, early_termination=False, **kwargs):
        num_obs = 0
        num_act = 0

        episode_length = int(self.episode_duration / self.frame_dt)
        super().__init__(num_envs, num_obs, num_act, episode_length, early_termination, **kwargs)

        self.action_scale = 1.0
        self.render_traj = True

        self.plotter = Plotter()
        self.plotter.gradient_labels = ["xd_grad", "omega_yaw_grad"]
        self.plotter.velocity_labels = ["xd_init", "omega_yaw_init"]

    def create_env(self, builder):
        # ke = 1.0e4
        # kf = 0.0
        # kd = 1.0e1
        # mu = 0.2

        self.block_width = 0.2
        self.block_length = 0.4
        self.block_height = 0.1

        block_ke = 1.0e6
        # block_kf = 0.0
        block_kf = 3e3
        block_kd = 1.0e1
        # block_mu = 0.0
        block_mu = 1.0

        ground_ke = block_ke
        ground_kf = block_kf
        ground_kd = block_kd
        ground_mu = block_mu
        self.ground_plane_settings = dict(ke=ground_ke, kd=ground_kd, kf=ground_kf, mu=ground_mu)

        # Dummy shape due to rendering bug
        builder.add_shape_box(body=-1, pos=wp.vec3(400.0, 1.0, 0.0), hx=0.25, hy=1.0, hz=1.0)

        block = builder.add_body(origin=wp.transform((0.0, self.block_height, 0.0), wp.quat_identity()), m=5.0)
        builder.add_shape_box(body=block, pos=wp.vec3(0.0, 0.0, 0.0),
                              hx=self.block_length, hy=self.block_height, hz=self.block_width,
                              ke=block_ke, kf=block_kf, kd=block_kd, mu=block_mu)

        builder.add_joint_free(child=block)
        # builder.joint_qd[3:6] = [5.0, -5.0, 0.0]
        builder.joint_q[0:3] = [0.0, self.block_height, 0.0]

        # Slight initial velocity
        # builder.body_qd[0] = (0.0, 0.5, 0.0, 0.01, 0.0, 0.0)

    def create_model(self):
        model = super().create_model()

        ke = 1.0e4
        kf = 0.0
        kd = 1.0e1
        mu = 0.2

        model.soft_contact_ke = ke
        model.soft_contact_kf = kf
        model.soft_contact_kd = kd
        model.soft_contact_mu = mu
        model.soft_contact_margin = 10.0
        model.soft_contact_restitution = 1.0

        return model

    def init_sim(self):
        super().init_sim()
        self.print_model_info()

        with torch.no_grad():
            self.target_pos = (3.0, self.block_height, 2.0)
            self.target_pos = torch.tensor(self.target_pos, device=self.device)
            self.target_ori = torch.tensor(np.pi/2, device=self.device)

    def reset_idx(self, env_ids):
        super().reset_idx(env_ids)
        if self.early_termination:
            raise NotImplementedError
        else:
            self.traj_verts = []

    @torch.no_grad()
    def randomize_init(self, env_ids):
        pass

    def pre_physics_step(self, actions):
        pass

    def compute_observations(self):
        body_q = self.state.joint_q.clone().view(self.num_envs, -1, 7)
        body_qd = self.state.joint_qd.clone().view(self.num_envs, -1, 6)
        # particle_q = self.state.particle_q.clone().view(self.num_envs, -1, 3)
        # particle_qd = self.state.particle_qd.clone().view(self.num_envs, -1, 3)

        body_q[:, :, :3] -= self.env_offsets.view(self.num_envs, 1, 3)
        # particle_q -= self.env_offsets.view(self.num_envs, 1, 3)

        delta_pos = body_q[:, 0, :3] - self.target_pos
        # delta = particle_q[:, 0, :] - self.target_pos

        # Convert from quaternion to pitch, then take the difference wrt target
        body_rot_quat = body_q[:, 0, 3:]
        w, x, y, z = body_rot_quat[:, 0], body_rot_quat[:, 1], body_rot_quat[:, 2], body_rot_quat[:, 3]
        t2 = 2.0 * (w * y - z * x)
        t2 = torch.clamp(t2, -1.0, 1.0)
        pitch = torch.asin(t2)

        delta_ori = pitch - self.target_ori
        delta_ori = torch.atan2(torch.sin(delta_ori), torch.cos(delta_ori))

        self.obs_buf = {
            "body_q": body_q,
            "body_qd": body_qd,
            "delta_pos": delta_pos,
            "delta_ori": delta_ori,
        }

    def compute_reward(self):
        delta_pos = self.obs_buf["delta_pos"]
        loss_pos = torch.einsum("bi,bi->b", delta_pos, delta_pos)  # dot product

        delta_ori = self.obs_buf["delta_ori"]
        loss_ori = torch.dot(delta_ori, delta_ori)

        # print(loss_pos, loss_ori)

        loss = loss_pos + loss_ori
        rew = -1.0 * loss

        reset_buf, progress_buf = self.reset_buf, self.progress_buf
        max_episode_steps, early_termination = self.episode_length, self.early_termination
        truncated = progress_buf > max_episode_steps - 1
        reset = torch.where(truncated, torch.ones_like(reset_buf), reset_buf)
        if early_termination:
            raise NotImplementedError
        else:
            terminated = torch.where(torch.zeros_like(reset), torch.ones_like(reset), reset)
        self.rew_buf, self.reset_buf, self.terminated_buf, self.truncated_buf = rew, reset, terminated, truncated

    def render(self, state=None):
        if not self.render_traj:
            super().render(state)
        else:
            # render state 1 (swapped with state 0 just before)
            state = state or self.state_1
            traj_vert = self.state.joint_q.view(self.num_envs, -1, 7)[:, 0, :3].tolist()
            # traj_vert = self.state.particle_q.view(self.num_envs, -1, 3)[:, 0, :].tolist()
            self.traj_verts.append(traj_vert)

            if self.renderer is not None:
                with wp.ScopedTimer("render", False):
                    self.render_time += self.frame_dt
                    self.renderer.begin_frame(self.render_time)
                    self.renderer.render(state)

                    traj_verts = np.array(self.traj_verts).transpose(1, 0, 2)  # -> B, T, 3
                    for i in range(self.num_envs):
                        if self.progress_buf[i] == 0:
                            continue

                        pos = (self.target_pos + self.env_offsets[i]).tolist()
                        self.renderer.render_box(
                            pos=pos,
                            rot=wp.quat_rpy(0.0, self.target_ori.item(), 0.0),
                            extents=(self.block_length, self.block_height, self.block_width),
                            # extents=(0.1, 0.1, 0.1),
                            color=(0.0, 1.0, 0.0),
                            name=f"target{i}",
                        )

                        if traj_verts.shape[1] > 1:
                            self.renderer.render_line_strip(
                                vertices=traj_verts[i],
                                color=wp.render.bourke_color_map(0.0, getattr(self, "max_iter", 250.0), self.iter),
                                radius=0.02,
                                name=f"iter{self.iter}_traj{i}",
                            )

                self.renderer.end_frame()

    def check_grad(self):
        raise NotImplementedError

    def run_cfg(self):
        assert self.requires_grad

        state_0 = self.state_0
        body_q_0, body_qd_0 = self.state_tensors

        # Only optimize over y angular velocity (index 1) and x linear velocity (index 3)
        def grad_mask_hook(grad):
            mask = torch.zeros_like(grad)
            mask[..., 1] = 1  # angular y
            mask[..., 3] = 1  # linear x
            mask[..., 5] = 1  # linear z
            return grad * mask

        # Register the hook on body_qd_0 so that only the allowed components get gradients
        body_qd_0.register_hook(grad_mask_hook)

        train_iters = 128
        train_rate = 0.1
        opt = torch.optim.SGD([body_qd_0], lr=train_rate)
        opt.add_param_group({"params": [body_q_0], "lr": 0})  # frozen params, adding so grads are zeroed

        self._run_state = state_0
        self._run_state_tensors = (body_q_0, body_qd_0)

        policy = None

        return train_iters, opt, policy

    def run_reset(self):
        obs = self.reset(clear_grad=self.requires_grad)

        state_0 = self._run_state
        body_q_0, body_qd_0 = self._run_state_tensors

        self.state_0 = state_0
        self.state_tensors = [body_q_0, body_qd_0]
        self.render_traj = self.iter % 16 == 0

        return obs

    def run_loss(self, traj, opt, policy):
        obses, actions, rewards, dones, infos = traj

        loss = -rewards[-1]

        opt.zero_grad()
        loss.sum().backward()
        opt.step()

        body_q_0, body_qd_0 = self._run_state_tensors

        # print(body_qd_0)

        print(f"Iter: {self.iter} Loss: {loss.tolist()}")
        print(f"Grad body q, qd: {body_q_0.grad.norm().item()}, {body_qd_0.grad.norm().item()}")

        # Store loss and gradient for plotting
        self.plotter.add_loss(loss.tolist()[0])
        self.plotter.add_gradient(body_qd_0.grad.detach().cpu().numpy().flatten()[[3, 1]])
        self.plotter.add_initial_velocity(body_qd_0.detach().cpu().numpy().flatten()[[3, 1]])


if __name__ == "__main__":
    run_env(Slide, no_grad=False)
