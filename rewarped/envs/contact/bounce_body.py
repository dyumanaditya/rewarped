...
class Bounce(WarpEnv):
    ...
    def __init__(self, num_envs=8, episode_length=-1, early_termination=False, **kwargs):
        ...
        if self.integrator_type == IntegratorType.FEATHERSTONE:
            self.state_tensors_names = ("joint_q", "joint_qd")
        else:
            self.state_tensors_names = ("body_q", "body_qd")
    ...
    def create_env(self, builder):
        ...
        if self.integrator_type == IntegratorType.FEATHERSTONE:
            ...
            builder.add_joint_free(child=sphere)
    ...
    def compute_observations(self):
        if self.integrator_type == IntegratorType.FEATHERSTONE:
            joint_q = self.state.joint_q.clone().view(self.num_envs, -1, 7)
            joint_qd = self.state.joint_qd.clone().view(self.num_envs, -1, 6)
    ...
    def run_loss(self, traj, opt, policy):
        ...
        print(body_qd_0.grad)
        print(f"Grad qd: {body_qd_0.grad[0][3].item()}, {body_qd_0.grad[0][4].item()}")
